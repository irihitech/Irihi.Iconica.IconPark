...
foreach (var svgFile in svgFiles)
{
    try
    {
        ...
        var baseClass = hasStroke ? "TDesignIconBase" : "TDesignIconFilledBase";

        var sourceCode = GenerateDocument(className, baseClass, elements);
...
void WalkElements(XmlElement parent, string? inheritedLayer, List<DrawingElement> result)
{
    foreach (var node in parent.ChildNodes)
    {
        ...
        var fillIndex = -1;
        var strokeIndex = -1;
        if (TryGetLayerNumber(effectiveLayer, "fill", out var fillN)) fillIndex = fillN * 2 - 1;
        else if (fill.Length > 0 && fill != "none") fillIndex = 1;
        if (TryGetLayerNumber(effectiveLayer, "stroke", out var strokeN)) strokeIndex = strokeN * 2 - 2;
...
static string GenerateDocument(string className, string baseClass, List<DrawingElement> elements)
{
    var sb = new StringBuilder();
...
static string GenerateDemoDocument(List<IconInfo> icons)
{
    var sb = new StringBuilder();
    sb.AppendLine("// AutoGenerated by IRIHI Iconica TDesign IconGenerator");
    sb.AppendLine("// Design CopyRight 2025 TDesign Authors");
    sb.AppendLine("// Generated CopyRight 2025 IRIHI Technology");
    sb.AppendLine("// Generated Date: " + DateTime.Today.ToString("yyyy-MM-dd", CultureInfo.InvariantCulture));
